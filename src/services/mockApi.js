...
export const getRideRequests = () => read('campuspool_ride_requests', [])
export const createRideRequest = (request) => { const requests = [{ ...request, id: `request-${Date.now()}`, status: 'Pending', createdAt: new Date().toISOString() }, ...getRideRequests()]; return write('campuspool_ride_requests', requests) }
export const cancelRideRequest = (requestId) => write('campuspool_ride_requests', getRideRequests().map(request => request.id === requestId ? { ...request, status: 'Cancelled' } : request))
